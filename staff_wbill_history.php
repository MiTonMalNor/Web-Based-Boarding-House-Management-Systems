						...
						<div class="row">
							<table class="table table-bordered">
								...
								<tbody>
									...
									$i = 1;
									$tamount = 0;
									$payments  = $conn->query("SELECT * FROM water_bill where date_format(due_date,'%Y-%m') = '$month_of' order by unix_timestamp(due_date) asc;");
									if($payments->num_rows > 0 ):
									while($row=$payments->fetch_assoc()):
										$tamount += $row['wbill_amount'];
									?>
									<tr>
										<td class="text-center"><?php echo $i++ ?></td>
										<td class="text-center"><?php echo date('M d, Y',strtotime($row['due_date'])) ?></td>
										<td class="text-center">₱<?php echo number_format($row['wbill_amount'],2) ?></td>
									</tr>
								<?php endwhile; ?>
								<?php else: ?>
									<tr>
