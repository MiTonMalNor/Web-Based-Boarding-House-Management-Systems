					...
					<div class="card-body">
						<table class="table table-condensed table-bordered table-hover">
							...
							<tbody>
								...
								<?php 
								$i = 1;
								$tenant = $conn->query("SELECT pt.*,concat(pt.fullname) as name,b.monthly_rate,b.bed_no,b.room_id FROM ptenants pt inner join beds b on b.id = pt.bed_id WHERE fullname not in (SELECT name FROM tenants) AND bed_id not in (SELECT bed_id FROM tenants) order by b.bed_no desc;");
